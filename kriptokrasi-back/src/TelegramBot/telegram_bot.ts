...
import { BUTTON_LIST, HELP_TEXT, trade_egitimi, vadeli_egitimi } from "../keyboards/consts";
...
class TelegramBot {
    ...
    registerCallbacks() {
        ...
        this.bot.hears(BUTTON_LIST.INITIAL, async (ctx) => {
            ...
            switch (message) {
                ...
                case BUTTON_LIST.INITIAL[1]:
                    let reply2 = "Aktif emir bulunmamaktadır.";
                    ctx.reply(reply2, { reply_markup: KEYBOARDS.INITIAL });
                    break;
                case BUTTON_LIST.INITIAL[2]:
                    let reply3 = "Geçmiş emir bulunmamaktadır.";
                    ctx.reply(reply3, { reply_markup: KEYBOARDS.INITIAL });
                    break;
        ...
        this.bot.hears(/(?<=fromto ).*/, async (ctx) => {
            const message = ctx.message.text;
